...
const SpecValue = ({ text }) => {
  ...
  return (
    <span className="font-semibold text-gray-800 text-right break-keep">
      {text}
    </span>
  );
};
    ...
    <div className="flex w-64 flex-shrink-0 flex-col space-y-3 rounded-xl bg-white p-5 shadow-md">
      ...
      <div className="space-y-2" style={{ fontSize: captionFont }}>
        <div className="flex justify-between items-start gap-2">
          <span className="font-medium text-gray-500 break-keep">데이터</span>
          <SpecValue text={plan.data} />
        </div>
        <div className="flex justify-between items-start gap-2">
          <span className="font-medium text-gray-500 break-keep">음성통화</span>
          <SpecValue text={plan.voiceCall} />
        </div>
        <div className="flex justify-between items-start gap-2">
          <span className="font-medium text-gray-500 break-keep">문자</span>
          <SpecValue text={plan.sms} />
        </div>
        {plan.tetheringAndSharing && (
          <div className="flex justify-between items-start gap-2">
            <span className="font-medium text-gray-500 break-keep">테더링 쉐어링</span>
